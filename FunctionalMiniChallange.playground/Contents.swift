...
var array1: [Student] = students.filter{$0.year == StudentYear.freshman}
...
var array2: [String] = students.map{$0.firstName + " " + $0.lastName}

// TODO: - Return the average grade from all the students using the reduce function divided by the count

let avg:Double = students.reduce(0.0) { $0.grade + (Double($1.grade) / Double(students.count)) }
